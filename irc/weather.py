...
# Imports, using all standard libraries except for BeautifulSoup
import urllib
import urllib2
import json
...
def grabinfo(cityoveride, countryoveride, overide):
    ...
    '''
    latlonglkup = urllib.urlopen(
        "https://maps.googleapis.com/maps/api/geocode/json?address= {0}+{1} &key= {2}".format(city, country,
                                                                                              api_key)).read()
    jdata = json.loads(latlonglkup)
    try:
        lat, lng = (
            jdata["results"][0]["geometry"]["location"]["lat"], jdata["results"][0]["geometry"]["location"]["lng"])
    except:
    ...
    edit("Local weather for {0} in local area ({1},{2}) is...".format(strftime("%a, %d %b %Y %H:%M:%S", localtime()),
                                                                      city, country))
